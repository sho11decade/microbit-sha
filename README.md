...
> **補足**: 以前のバージョンでは `requestDevice` の `filters` に NUS サービス UUID フィルタが含まれており、一部の OS/ブラウザ（特に Windows 上の Chrome）でサービス UUID がアドバタイズパケットに含まれない場合にデバイスが一覧表示されないことがありました。現在は名前フィルタのみを使用するように修正されています。それでも表示されない場合は「全デバイス表示」ボタンをお試しください。
